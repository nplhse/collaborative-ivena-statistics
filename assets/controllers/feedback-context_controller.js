...
export default class extends Controller {
    ...
    sync () {
        ...
        const metaParams = document.querySelector('meta[name="feedback-return-route-params"]')?.content

        if (this.hasSourceRouteTarget) {
            const route = metaRoute && metaRoute !== '' ? metaRoute : (this.hasRouteValue ? this.routeValue : '')
            if (route !== '') {
        ...
        if (this.hasSourceRouteParamsTarget) {
            const params = metaParams && metaParams !== '' ? metaParams : (this.hasRouteParamsValue ? this.routeParamsValue : '{}')
