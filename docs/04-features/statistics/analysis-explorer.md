...
Under the chart title, the explorer shows a short human-readable summary of the **effective** analysis configuration (what is measured, for which population, which period, how it is grouped, and important filters). It is generated by `ExplorerAnalysisSummaryFactory` from the same normalized `AnalysisViewConfig` used for the query/result, and updates on every `rerunAnalysis()` / chart-row-limit change.

| Concern | Implementation |
|---|---|
| View model | `AnalysisSummaryViewModel` + `AnalysisSummaryPart` (plain vs emphasized text) |
| Labels | `ExplorerAnalysisSummaryLabelResolver` (scope/period); filter labels via `ExplorerFilterBadgePresenter` |
| i18n | `stats.analysis_explorer.summary.*` in `statistics+intl-icu.{en,de}.xlf` — no HTML in translations |
...
- Live action `setChartRowLimit` rebuilds chart specs and the natural-language summary (no DB re-query).
