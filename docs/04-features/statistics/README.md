...
| Top Lists | `/statistics/top-lists` | Catalog of ranked tables; detail routes under `/statistics/top-lists/{report}` (ranking depth Top 10–100 or All, page size 25/50/100, optional Scope/Period comparison). Catalogue list/detail pages for indications, departments, specialities, assignments, occasions, and infections link here; rows link to Explore show pages. Secondary diagnoses share the indication catalogue. No top list exists for groups, hospitals, states, dispatch areas, secondary transports, or glossary terms. |
