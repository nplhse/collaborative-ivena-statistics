...
final class DemoteUserFromAdminCommandTest extends TestCase
{
    public function testReturnsFailureWhenDemoteLosesTheLastAdministrator(): void
    {
        $user = new User()->setUsername('alice')->setRoles([UserRole::ADMIN]);

        $service = $this->createMock(AdministrativeUserService::class);
        $service->method('findByUsername')->with('alice')->willReturn($user);
        $service->method('countAdministrators')->willReturn(2);
        $service->method('demoteFromAdmin')->willThrowException(new LastAdministratorException());

        $io = $this->createMock(SymfonyStyle::class);
        $io->method('ask')->willReturn('alice');
        $io->method('confirm')->willReturn(true);
        $io->expects(self::once())->method('error')->with(LastAdministratorException::MESSAGE);

        $status = new DemoteUserFromAdminCommand($service, $this->acceptingValidator())(
            $io,
    ...
    public function testReturnsFailureWhenUsernameAnswerIsNotAString(): void
    {
        ...
        $status = new DemoteUserFromAdminCommand(
