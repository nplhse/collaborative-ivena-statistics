...
final readonly class HospitalMatcher
{
    ...
    public function matchOrFail(int $legacyId, string $legacyName, array $hospitals): array
    {
        ...
        if ([] === $candidates) {
            throw new \RuntimeException(sprintf('No hospital match for legacy id %d ("%s", normalized: "%s").', $legacyId, $legacyName, $normalized));
        }

        if (isset($candidates[1]) && abs($candidates[0]['score'] - $candidates[1]['score']) < 0.02) {
            throw new \RuntimeException(sprintf('Ambiguous hospital match for legacy id %d ("%s", normalized: "%s"). Top scores: %s / %s.', $legacyId, $legacyName, $normalized, number_format($candidates[0]['score'], 3), number_format($candidates[1]['score'], 3)));
        }
