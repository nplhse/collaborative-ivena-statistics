...
final readonly class BenchmarkComparisonPageViewModelFactory
{
    ...
    public function create(Request $request, string $routeName, ?User $user, StatisticsFilter $filter): StatisticsPageViewModel
    {
        ...
        $scopeUrls = [
            'public' => $this->statisticsNavigationUrlBuilder->build(
                ...
                StatisticsQueryKeys::REMOVE_COMPARISON_SCOPE_DEPENDENT,
            ),
            'my_hospitals' => $this->statisticsNavigationUrlBuilder->build(
                $request,
                $routeName,
                [StatisticsQueryKeys::COMPARISON_SCOPE => StatisticsFilterScope::MyHospitals->value],
                StatisticsQueryKeys::REMOVE_COMPARISON_SCOPE_DEPENDENT,
            ),
        ];
            ...
            foreach ($accessibleHospitals as $row) {
                $hospitalUrls[(string) $row['id']] = $this->statisticsNavigationUrlBuilder->build(
                    ...
                    [
                        ...
                        StatisticsQueryKeys::COMPARISON_HOSPITAL => $row['id'],
                    ],
                );
            }
        }
        ...
        $periodUrls = [
            'all' => $this->statisticsNavigationUrlBuilder->build(
                ...
                StatisticsQueryKeys::REMOVE_COMPARISON_PERIOD_DEPENDENT,
            ),
            ...
            'year' => $this->statisticsNavigationUrlBuilder->build(
                ...
                [
                    ...
                    StatisticsQueryKeys::COMPARISON_YEAR => $defaultYear,
                ],
                StatisticsQueryKeys::REMOVE_COMPARISON_MONTH_DEPENDENT,
            ),
            'month' => $this->statisticsNavigationUrlBuilder->build(
                $request,
                $routeName,
                [
                    StatisticsQueryKeys::COMPARISON_PERIOD => StatisticsFilterPeriod::Month->value,
                    StatisticsQueryKeys::COMPARISON_YEAR => $defaultYear,
                    StatisticsQueryKeys::COMPARISON_MONTH => $defaultMonth,
                ],
            ),
            'quarter' => $this->statisticsNavigationUrlBuilder->build(
                ...
                $routeName,
                [
                    StatisticsQueryKeys::COMPARISON_PERIOD => StatisticsFilterPeriod::Quarter->value,
                    StatisticsQueryKeys::COMPARISON_YEAR => $defaultYear,
                    StatisticsQueryKeys::COMPARISON_QUARTER => (int) ceil($defaultMonth / 3),
                ],
                StatisticsQueryKeys::REMOVE_COMPARISON_MONTH_DEPENDENT,
            ),
        ];
        ...
        foreach (HospitalCohortKey::all() as $cohortKey) {
            ...
            $cohortScopeChoices[] = [
                ...
                'url' => $this->statisticsNavigationUrlBuilder->build(
                    ...
                    $routeName,
                    [
                        StatisticsQueryKeys::COMPARISON_SCOPE => StatisticsFilterScope::HospitalCohort->value.':'.$cohortKey->value(),
                    ],
                    StatisticsQueryKeys::REMOVE_COMPARISON_SCOPE_DEPENDENT,
                ),
    ...
    private function buildStateScopeUrl(Request $request, string $routeName, int $stateId): string
    {
        return $this->statisticsNavigationUrlBuilder->build(
            ...
            $routeName,
            [
                StatisticsQueryKeys::COMPARISON_SCOPE => StatisticsFilterScope::State->value.':'.$stateId,
            ],
            StatisticsQueryKeys::REMOVE_COMPARISON_SCOPE_DEPENDENT,
        );
    }
    ...
    private function buildDispatchAreaScopeUrl(Request $request, string $routeName, int $dispatchAreaId): string
    {
        return $this->statisticsNavigationUrlBuilder->build(
            ...
            $routeName,
            [
                StatisticsQueryKeys::COMPARISON_SCOPE => StatisticsFilterScope::DispatchArea->value.':'.$dispatchAreaId,
            ],
            StatisticsQueryKeys::REMOVE_COMPARISON_SCOPE_DEPENDENT,
        );
    }
