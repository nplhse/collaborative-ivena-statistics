...
final readonly class OverviewPeriodViewModelFactory
{
    ...
    {
        $params = [StatisticsQueryKeys::PERIOD => $filter->period->value];

        if (null !== $filter->referenceYear) {
            $params[StatisticsQueryKeys::YEAR] = $filter->referenceYear;
        }
        if (null !== $filter->referenceMonth) {
            $params[StatisticsQueryKeys::MONTH] = $filter->referenceMonth;
        }
        if (null !== $filter->referenceQuarter) {
            $params[StatisticsQueryKeys::QUARTER] = $filter->referenceQuarter;
        }
    ...
    {
        return match ($filter->period) {
            StatisticsFilterPeriod::AllTime, StatisticsFilterPeriod::All => StatisticsQueryKeys::REMOVE_PERIOD_DEPENDENT,
            StatisticsFilterPeriod::Year => StatisticsQueryKeys::REMOVE_QUARTER_DEPENDENT,
            StatisticsFilterPeriod::Quarter => StatisticsQueryKeys::REMOVE_MONTH_DEPENDENT,
            StatisticsFilterPeriod::Month => [StatisticsQueryKeys::QUARTER],
        };
    }
