...
final class TimeGridChart
{
     ...
     *  label:string,
     *  format:string,
     *  cells:list<\App\Model\TimeGridCell|array{value?:int|float, compare?:int|float}>
     * }>
     */
    public array $rows = [];
    ...
    public function init(): void
    {
        ...
        foreach ($this->rows as $row) {
            ...
            foreach ($this->labels as $i => $_) {
                ...
                $v = ($cell instanceof \App\Model\TimeGridCell)
                    ? $cell->value
            ...
            if (TimeGridMode::COMPARE === $this->mode) {
                ...
                foreach ($this->labels as $i => $_) {
                    $cell = $row['cells'][$i] ?? null;
                    $c = ($cell instanceof \App\Model\TimeGridCell)
                        ? $cell->compare
